...
invoice-web은 **노션 데이터베이스를 유일한 데이터 소스**로 사용하는 견적서 공유 도구입니다. 별도 데이터베이스(Supabase 등) 없이, 관리자가 노션에서 직접 견적서를 작성하고 slug를 관리하면 클라이언트는 공유 링크로 조회 및 PDF 다운로드만 수행합니다:

- **slug로 견적서 조회**: 노션 데이터베이스에서 slug → page_id 매핑 조회 (F001)
- **노션 페이지 실시간 렌더링**: 환경변수의 Notion API Key로 페이지 블록을 실시간으로 읽어 웹에 렌더링 (F002)
- **PDF 다운로드**: 클라이언트가 렌더링된 견적서를 PDF로 저장 (F003)

### 노션 데이터 구조

관리자가 노션에서 직접 관리하는 데이터베이스 (별도 DB 없음):

```
[노션 데이터베이스] — 견적서 목록
  - 제목 (Title)
  - slug (Text) → 공유 URL에 사용 (예: client-abc-2026)
  - page_id (Text) → 견적서 내용 페이지 ID

[노션 페이지] — 견적서 내용
  - 관리자가 자유롭게 작성한 견적서 본문
```
...
- [ ] Task 001: 프로젝트 라우트 구조 및 빈 페이지 골격 생성 - 우선순위
...
- [ ] Task 002: 타입 정의
  - `Invoice` TypeScript 인터페이스 정의 (`src/types/`) — 노션 DB 행 구조(slug, page_id, title) 반영
  - 노션 블록 타입 및 API 응답 타입 정의
  - 환경변수 타입 정의 (`NOTION_API_KEY`, `NOTION_DATABASE_ID`)
...
- [ ] Task 003: 공통 컴포넌트 및 더미 데이터 유틸리티 구현
  - shadcn/ui 기반 필요 컴포넌트만 추가 (Button, Card, Skeleton 등)
  - 노션 블록 렌더링용 도메인 컴포넌트 골격 (텍스트/표/이미지 블록)
  - 더미 데이터 유틸리티 작성 (더미 노션 블록 콘텐츠)
  - 토스트(sonner) 패턴 확립

- [ ] Task 004: 견적서 공개 조회 및 오류 페이지 UI 완성 (F002, F003)
  - 공개 조회 페이지 UI: 노션 블록 렌더링 영역(더미 콘텐츠), "PDF 다운로드" 버튼
  - 노션 텍스트/표/이미지 블록 표시용 컴포넌트 골격
...
- [ ] Task 005: 환경변수 설정 및 노션 API 클라이언트 구성 - 우선순위
  - 환경변수 구성 (`NOTION_API_KEY`, `NOTION_DATABASE_ID`)
  - `@notionhq/client` 서버 사이드 클라이언트 유틸 작성 (`src/lib/notion.ts`)
  - 노션 데이터베이스에서 slug로 page_id 조회 함수 구현 (F001)
  - **Playwright MCP로 유효/무효 slug 조회 동작 검증 E2E 테스트**

- [ ] Task 006: 노션 페이지 실시간 렌더링 구현 (F002)
  - slug → page_id 확보 후 Notion API로 페이지 블록 실시간 읽기 및 웹 렌더링 (react-notion-x 또는 notion-to-md)
  - 텍스트/표/이미지 등 주요 블록 타입 지원
  - 존재하지 않는 slug → 오류 안내 페이지 표시
  - **Playwright MCP로 유효 링크 렌더링 및 무효 링크 오류 페이지 E2E 테스트**

- [ ] Task 007: PDF 다운로드 구현 (F003)
  - "PDF 다운로드" 버튼 클릭 시 렌더링 내용을 PDF로 변환 (html2canvas + jsPDF)
  - 노션 콘텐츠 레이아웃 보존 및 다운로드 트리거
  - **Playwright MCP로 PDF 다운로드 동작 및 파일 생성 검증 E2E 테스트**
...
| 항목 | 후보 | 결정 기준 |
|------|------|-----------|
| 노션 블록 렌더링 | `react-notion-x` vs `notion-to-md` | react-notion-x는 노션 네이티브 스타일 재현에 강점, notion-to-md는 마크다운 변환 후 커스텀 렌더링 유연. 블록 타입 커버리지와 스타일 일관성 기준 결정 |
| PDF 생성 | `html2canvas + jsPDF` vs `@react-pdf/renderer` | html2canvas는 렌더링 화면 그대로 캡처(구현 단순), @react-pdf/renderer는 레이아웃 정밀 제어 가능하나 노션 스타일 재현 비용 큼. 품질/구현 비용 기준 결정 |
